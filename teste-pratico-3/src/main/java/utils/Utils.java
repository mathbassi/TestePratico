...
public class Utils {
	...
	public static void setEnvironment() {
		...
		File environmentPropertie = new File("C:\\Users\\mbassipa\\eclipse-workspace\\teste-pratico-3\\src\\test\\resources\\testsettings.properties");
		if (environmentPropertie.exists()) {
			environmentPropertiesPath = "C:\\Users\\mbassipa\\eclipse-workspace\\teste-pratico-3\\src\\test\\resources\\testsettings.properties";
		} else {
    ...
    public static List<Map<String, String>> listResultDBGenerator(ResultSet results, List<Map<String, String>> resultList, String[] collunms,  int qtdResults) throws SQLException {
        if(!results.next()){
            return resultList;
        }
        ...
        if (resultList.size() <= qtdResults){
            listResultDBGenerator(results,resultList,collunms,qtdResults);
        }
        return resultList;
    }
